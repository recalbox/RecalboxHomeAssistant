![ha_badge](https://img.shields.io/badge/Home_Assistant-2026.2.3_Compatible-blue)
![rb_badge](https://img.shields.io/badge/Recalbox-10.0.1_Compatible-blue)
[![hacs_badge](https://img.shields.io/badge/HACS-Custom-41BDF5.svg)](https://github.com/hacs/integration)  
[![Release](https://img.shields.io/github/v/release/recalbox/RecalboxHomeAssistant)](https://github.com/recalbox/RecalboxHomeAssistant/releases)
![GitHub issue custom search](https://img.shields.io/github/issues-search?query=is%3Aopen%20repo%3Arecalbox%2FRecalboxHomeAssistant&label=Open%20issues)

# Intégration Recalbox Home Assistant

<small>Par Recalbox, 2026</small>

🇫🇷 Version Française  
[🇺🇸 Go to English version](README.md)

<img src="logo.png" height="196px">

Ce dépôt vous permet d'intégrer Recalbox dans votre Home Assistant :
- Dans votre tableau de bord :
  - Voir le status
  - Afficher le jeu en cours
  - Arrêter le jeu
  - Prendre une capture d'écran
  - Pause/reprendre l'émulateur
  - Enregistrer/charger la partie
  - Eteindre
  - Redémarrer
  - etc
- Déclencher vos automatisations.  
  Par exemple, changer la couleur des lumières selon le jeu lancé, envoyer des notifications, etc.
- Commandes vocales/textuelles Assist (EN/FR) :
  - Lancer un jeu par son titre (complet ou partiel)
  - Demander quel est le jeu en cours
  - Arrêter le jeu en cours
  - Demander une capture d'écran
  - Pause/reprendre le jeu
  - Enregistrer/charger la partie
  - Eteindre la Recalbox
  - etc

![](docs/RecalboxHomeAssistantIntegration.png)


- [Pré-requis](https://github.com/recalbox/RecalboxHomeAssistant/wiki/FR_Home#requirements)
- [Installation](https://github.com/recalbox/RecalboxHomeAssistant/wiki/FR_Installation)
- Usage :
  * [Tableau de bord](https://github.com/recalbox/RecalboxHomeAssistant/wiki/FR_Usage-on-Lovelace-Dashboard)
  * [Automatisations](https://github.com/recalbox/RecalboxHomeAssistant/wiki/FR_Usage-with-automations)
  * [Assist (texte/voix)](https://github.com/recalbox/RecalboxHomeAssistant/wiki/FR_Usage-with-Assist)
- [Notes de versions](CHANGELOG.md)
- [Aides](https://github.com/recalbox/RecalboxHomeAssistant/wiki/FR_Troubleshot)




